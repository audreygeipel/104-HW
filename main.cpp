...
int main(int argc, char** argv)
{
	...
	for(size_t playerNum = 1; playerNum <= numPlayers; ++playerNum)
	{
		...
		if(playerName[0] == 'c' && playerName[1] == 'p' && playerName[2] == 'u' && playerName[3] == 'l'){
			// CPU with MAX LETTERS
			std::cout << "CPU " << playerNum << ", named \"" << playerName << "\", has been added." << std::endl;
			players.push_back(new Player(playerName, configFile->handSize, 1));
			players[playerNum - 1]->addTiles(bag->drawTiles(configFile->handSize));

		}
		else if(playerName[0] == 'c' && playerName[1] == 'p' && playerName[2] == 'u' && playerName[3] == 's'){
				// CPU with HIGHEST SCORE
			std::cout << "CPU " << playerNum << ", named \"" << playerName << "\", has been added." << std::endl;
			players.push_back(new Player(playerName, configFile->handSize, 2));
			players[playerNum - 1]->addTiles(bag->drawTiles(configFile->handSize));
			
		}
		else{
			// create human player with a full initial hand
			std::cout << "Player " << playerNum << ", named \"" << playerName << "\", has been added." << std::endl;
			players.push_back(new Player(playerName, configFile->handSize, 0));
			players[playerNum - 1]->addTiles(bag->drawTiles(configFile->handSize));
		}
...
//			ConsolePrinter::printHand(*players[playerNum]);
		...
		if(players[playerNum]->getType() == 0){//if it's a human player
			ConsolePrinter::printHand(*players[playerNum]);
		...
		}// end of human-player if
		if(players[playerNum]->getType() == 1){

			std::cout << std::endl;
			
			std::cout << players[playerNum]->getName()<< " played" << std::endl;

			//INSERT THE MOVE HERE
			std::vector<std::string> possiblemoves = players[playerNum]->findWords(players[playerNum]->getHand(), board, dictionary);

//CPULmove(std::vector<std::string> w, Board * board, Bag * bag, Dictionary * dictionary){
			players[playerNum]->CPULmove(possiblemoves, board, bag, dictionary);

			// draw more tiles from the bag to bring the player up to a full hand
			std::vector<Tile *> newTiles = bag->drawTiles(players[playerNum]->getMaxTiles() - players[playerNum]->getNumTiles());
			players[playerNum]->addTiles(newTiles);
			std::cout << "They picked up new tiles." << TILE_NAME_COLOR << std::endl;

			std::cout << "Their current score: " << SCORE_COLOR << players[playerNum]->getPoints() << rang::style::reset << std::endl;

			}
		if(players[playerNum]->getType() == 2){
				...
				//INSERT THE MOVE HERE
				players[playerNum]->findWords(players[playerNum]->getHand(), board, dictionary);
