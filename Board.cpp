...
int j = 0;// this will be the row number
	std::string row;

while(getline(initFileStream, row)){
	
	//getline(initFileStream, row);

	for(unsigned int i = 0; i<row.size(); i=i+3){
		//if it's not a period
	//	std::cout << (i/3) << " " << j+1 << " ";
		
		if(row[i] != '.'){
			//make a tile to place with the info
			char letter = tolower(row[i]);
			
			if(row[i+1] == '1'){
				unsigned int value = 10;
				Tile * t = new Tile(letter, value);
				//place it
				//SquareCoords here(i/3, j);
				getSquare((i/3)+1,j+1)->placeTile(t);
			}
			else{
				unsigned int value = row[i+2] - 48;
				Tile * t = new Tile(letter, value);
				//place it
				//SquareCoords here(i/3, j);
				getSquare((i/3)+1,j+1)->placeTile(t);
